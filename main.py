...
def register():
    ...
    if user:
        flash(f"That email exists, please log in here {user.email}")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            db.session.commit()
            return redirect(url_for("show_post", post_id=post_id, post=requested_post))
        else:
            flash("You must sign in to make a comment.")
            return redirect(url_for("login"))
